...
class UnetGenerator_MultiOutput(nn.Module):
    ...
    '''
    def __init__(self, input_nc, output_nc = [3], nf=64, max_nf=256, num_scales=7, n_residual_blocks=2, norm='batch', activation=nn.ReLU(False), use_dropout=False):
        ...
        for l in range(num_scales):
            ...
            for i in range(n_residual_blocks):
                self.__setattr__('enc_%d_res_%d'%(l, i), ResidualBlock2(c_in, None, norm_layer, use_bias, activation, use_dropout=False))
            ...
            for i in range(n_residual_blocks):
                self.__setattr__('dec_%d_res_%d'%(l, i), ResidualBlock2(c_in, c_in, norm_layer, use_bias, activation, use_dropout))
...
class BaseAttentionLWBGenerator(nn.Module, ABC):
    ...
    def forward_tsf(self, tsf_inputs, src_enc_outs, src_res_outs, Tst,
        ...
        for i in range(n_down):
            ...
            #[1, 64, 256, 256]  [4, 64, 256, 256]  [1, 4, 512, 512, 2]
...
class AttentionLWBpwGenerator(BaseAttentionLWBGenerator):

    def __init__(self, cfg, nc=6, temporal=False):
        """
        ...
        super(AttentionLWBpwGenerator, self).__init__()
        self._name = cfg.name
        ...
        # 4. build pixel warp net
        # print(cfg)
        nc = 15
        # self.pw_net = UnetGenerator_MultiOutput(input_nc=nc, output_nc=[1],
        #                                          nf=16, max_nf=32, 
        #                                          num_scales=3, n_residual_blocks=2,
        #                                          )
        self.pw_net = ResNetInpaintor(
            c_dim=nc,
            num_filters=[64,128,256],
            n_res_block=2
        )

        self.enc_src_attlwbs = SelfAttentionLWB(
            channel_q=6,
            channel_s=6,
            channel=6,
            temporal=temporal
    ...
    def pixel_warp(self, tsf_img, tsf_input, src_inputs, Tst):

        ns, c, h, w = src_inputs.shape  
        #[1, 6, 512, 512]  [4, 6, 512, 512]  [1, 4, 512, 512, 2]
        tsf_x = self.enc_src_attlwbs(tsf_input, src_inputs, Tst, temp_x=None, Ttt=None)
        #[1, 6, 512, 512]

        pw_input = torch.cat([tsf_img, tsf_input, tsf_x], dim=1)
        pw_out = self.pw_net(pw_input)
        pw_out = pw_out.mean(1,keepdim=True)
        # pw_out = pw_out.unsqueeze(0)
        # pw_out = pw_out.view(bs, nt, 1, h, w)
        pix_mask = F.sigmoid(pw_out)
    ...
    def forward(self, bg_inputs, src_inputs, tsf_inputs, Tst, Ttt=None, only_tsf=True):
        ...
        for t in range(nt):
            ...
            tsf_img, tsf_mask = self.forward_tsf(t_tsf_inputs, src_enc_outs, src_res_outs,
                                                 Tst[:, t].contiguous(), _temp_enc_outs, _temp_res_outs, _Ttt)
            #[1, 3, 512, 512]  [1, 6, 512, 512] [1, 4, 6, 512, 512]
            pix_mask = self.pixel_warp(tsf_img.data, t_tsf_inputs.data, src_inputs.data.view(bs*ns,6,h,w), Tst[:, t].contiguous())
            
            tsf_img = t_tsf_inputs[:,0:3].data * pix_mask + tsf_img * (1 - pix_mask)
            tsf_img = F.tanh(tsf_img)
            
            
            tsf_imgs.append(tsf_img)
            tsf_masks.append(tsf_mask)

        tsf_imgs = torch.stack(tsf_imgs, dim=1) # 1 2 3 512 512 是经过tanh的
        tsf_masks = torch.stack(tsf_masks, dim=1) #1 2 1 512 512

        #add pixel warp
        # tsf_imgs = F.tanh(tsf_imgs)

        # pw_out = self.pixel_warp(tsf_imgs.data, tsf_inputs.data, src_inputs.data)
        # pix_mask = F.sigmoid(pw_out)
        
        # tsf_imgs = tsf_inputs[:,:,0:3].data * pix_mask + tsf_imgs * (1 - pix_mask)
        # tsf_imgs = F.tanh(tsf_imgs)
