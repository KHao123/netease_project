...
class Imitator(BaseRunnerModel):
    ...
    def _create_networks(self):
        ...
        # create uvmap completion
        self.gen_uvmap = self._create_uvmap(self._opt.neural_render_cfg.Generator)
        self.gen_uvmap = self.gen_uvmap.to(self.device)

    def _create_uvmap(self, cfg):
        net = NetworksFactory.get_by_name("uv_map_completion", cfg=cfg, temporal=self._opt.temporal)
        load_path = '/home/chenkanghao/mywork/iPER/iPERCore_uvmap/experiments/models/uv_map_completion_3/net_iter_400000_id_G.pth'
    ...
    def source_setup(self, src_path, src_smpl, masks=None, bg_img=None, offsets=0, links_ids=None, visualizer=None):
        ...
        uv_img, input_G_bg, input_G_src, uv_maps, uv_vis_maps = self.flow_comp.process_source(src_img, src_info, primary_ids=[0])

        src_info["uv_img"] = uv_img.data

        bs, ns, c, h, w = 1, 1, 3, 512, 512
        fake_uv_maps = self.gen_uvmap(uv_maps.view(bs*ns, c, h, w)) #没有sigmod
        uv_vis_maps = uv_vis_maps.view(bs*ns, 1, h, w).data
        uv_img = uv_maps.view(bs*ns, c, h, w) * uv_vis_maps + fake_uv_maps * (1-uv_vis_maps)
        src_info["fake_uv_img"] = fake_uv_maps 
        src_info["complete_uv_img"] = uv_img
        #是否用complete uv img去生成
        ...
        if visualizer is not None:
            ...
            visualizer.vis_named_img("complete_uv_map", src_info["complete_uv_img"])
            visualizer.vis_named_img("fake_uv_img", src_info["fake_uv_img"])
