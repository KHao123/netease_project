...
class uv_map_completion(nn.Module):
	def __init__(self, num_classes=3, in_channels=3, depth=4,
		...
		Arguments:
			...
			up_mode: string, type of upconvolution. Choices: 'transpose'
				...
				upsampling.
		"""
		super(uv_map_completion, self).__init__()
	...
	def forward(self, x):
		...
		x = self.conv_final(x)
        # x = F.sigmoid(x)
		# loss = self.abs_loss(input_,x[:,:3,:,:]) + self.mse_loss(input_, x[:,:3,:,:]) #在前3个channel 保留原uv map的信息
		return x
