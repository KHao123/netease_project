...
class LWGTrainerABC(BaseTrainerModel, abc.ABC):
    ...
    def save(self, label):
        ...
        if "module" in self.G.__dict__:
            self.save_network(self.G.module, "G", label) #这里已经保存module了
...
class LWGAugBGTrainer(LWGTrainerABC):
    ...
    def set_input(self, inputs, device):
        ...
        with torch.no_grad():
            ...
            ################################################################
            input_G_bg, input_G_src, input_G_tsf, Tst, Ttt, src_mask, tsf_mask, head_bbox, body_bbox, uv_img, _, _ = \
...
class LWGTrainer(LWGTrainerABC):
    ...
    def optimize_G(self, fake_bg, fake_src_imgs, fake_tsf_imgs, fake_masks):
        ...
        if self._use_gan:
            ...
            # gan loss
            d_fake_outs = self.D(d_inputs)
            self._loss_g_adv = self.crt_gan(d_fake_outs, 0) * self._train_opts.lambda_D_prob #为什么是0
...
class LWGFrontTrainer(LWGTrainer):
    ...
    def set_input(self, inputs, device):
        with torch.no_grad():
            ...
            _, input_G_src, input_G_tsf, Tst, Ttt, src_mask, tsf_mask, head_bbox, body_bbox, uv_img,_ ,_ = \
                self.flow_comp(src_img, tsf_img, src_smpl, tsf_smpl, src_mask=src_mask, ref_mask=ref_mask,
...
class UVmapTraniner(LWGTrainerABC):
    """
    This is the LWGTrainer for personalization. In this class, the `inputs` in self.set_input(inputs, device)
    provides a `bg`. We use it to fine-tune the BGNet for a better background inpainting result.
    However, in the most cases, there are not actual background images provided.
    ...
    def __init__(self, opt, device):
        super(UVmapTraniner, self).__init__(opt, device)
        self._name = "UVmapTraniner"
    ...
    def set_input(self, inputs, device):
        ...
        with torch.no_grad():
            ...
            # src_img = images[:, :].contiguous()
            # src_smpl = smpls[:, :].contiguous()
            # src_mask = masks[:, :].contiguous()
            # self._opt.num_source = src_img.shape[1]

            # all_image = images.contiguous()
            ...
            input_G_bg, input_G_src, input_G_tsf, Tst, Ttt, src_mask, tsf_mask, head_bbox, body_bbox, uv_img, uv_maps, uv_vis_maps = \
                self.flow_comp(src_img, tsf_img, src_smpl, tsf_smpl, src_mask=src_mask, ref_mask=ref_mask,
                               links_ids=links_ids, offsets=offsets, temporal=self._opt.temporal)
    ...
    def optimize_parameters(self, trainable=True, keep_data_for_visuals=False):
        ...
        # run inference
        full_uv_maps = self.forward(
            keep_data_for_visuals=keep_data_for_visuals)

        # train G
        if trainable:
            loss_G = self.optimize_G(full_uv_maps)
            self._optimizer_G.zero_grad()
            loss_G.backward()
            self._optimizer_G.step()

        if self._use_gan:
            loss_D = self.optimize_D(full_uv_maps)
            self._optimizer_D.zero_grad()
            loss_D.backward()
            self._optimizer_D.step()
    ...
    def forward(self, keep_data_for_visuals=False, return_estimates=False):
        ...
        """
        bs, ns, c, h, w = self.uv_maps.shape
        fake_uv_maps = self.G(self.uv_maps.view(bs*ns, c, h, w))
        #是否需要用uv_vis_maps处理一下
        uv_vis_maps = self.uv_vis_maps.view(bs*ns, 1, h, w).data

        self.real_uv_vis_maps = uv_vis_maps.mean(0)


        fake_uv_maps = self.uv_maps.view(bs*ns, c, h, w).data * uv_vis_maps + fake_uv_maps * (1-uv_vis_maps) #可见区域直接输出，只预测不可见区域


        if keep_data_for_visuals:
            self.visual_imgs(fake_uv_maps)
    ...
    def optimize_G(self, fake_uv_maps):
        """
        ...
        ns, c, h, w = fake_uv_maps.shape
        
        fake_uv_maps = fake_uv_maps.view(-1, 3, h, w) #6, 3, 512, 512 uv_vis_maps = 1, 6, 1, 512, 512
        real_uv_map = self._uv_img.repeat(ns, 1, 1, 1).detach()
        #算损失的时候加入vis是的输出的可见部分尽量相近
        # uv_vis_maps = self.uv_vis_maps.view(ns, 1, h, w).data
        # fake_uv_maps = fake_uv_maps.data * uv_vis_maps + fake_uv_maps * (1-uv_vis_maps)
        fake_uv_maps = fake_uv_maps * self.real_uv_vis_maps  #如果全都没有的区域就不用回归
        real_uv_map  = real_uv_map * self.real_uv_vis_maps
        ...
        if self._use_gan:
            ...
            d_fake_outs = self.D(fake_inputs)
            self._loss_g_adv = self.crt_gan(d_fake_outs, 0) * self._train_opts.lambda_D_prob

        self._loss_g_rec = self.crt_l1(fake_uv_maps, real_uv_map) * self._train_opts.lambda_tsf

        # if self._train_opts.use_face:
            # self._loss_g_face = self.crt_face()

        return self._loss_g_adv + self._loss_g_rec
    ...
    def optimize_D(self, fake_uv_maps):
        """
        ...
        """
        ns, c, h, w = fake_uv_maps.shape
        real_uv_map = self._uv_img.view(-1, 3, h, w)

        real_inputs = {
            "x": real_uv_map.detach(),
            "bg_x": None,
            "get_avg": False
        }
        d_real_outs = self.D(real_inputs)


        fake_inputs = {
            "x": fake_uv_maps.detach(),
            "bg_x": None,
            "get_avg": False
        }
        d_fake_outs = self.D(fake_inputs)
        ...
        return _loss_d_fake + _loss_d_real
    ...
    @torch.no_grad()
    def visual_imgs(self, fake_uv):
        self._vis_fake_uv = tensor2im(fake_uv, idx=-1)
        self._vis_real_uv = tensor2im(self._uv_img[0], idx=-1)
        self._vis_source = tensor2im(self._real_src[0], idx=-1)
        self._vis_sourceuv = tensor2im(self.uv_maps[0], idx=-1)
        self._vis_sourceuvmask = tensor2im(self.uv_vis_maps[0], idx=-1)
        # fake_real_uvmap = fake_uv.
    ...
    def get_current_visuals(self):
        ...
        visuals["0_sourceuv"] = self._vis_sourceuv
        visuals["1_targetuv"] = self._vis_real_uv
        visuals["2_sourceuvmask"] = self._vis_sourceuvmask
        visuals["3_fake_uv"] = self._vis_fake_uv
        visuals["4_source_img"] = self._vis_source
