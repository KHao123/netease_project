...
size = (1000,1002)#这个是图片的尺寸，一定要和要用的图片size一致
...
video_name = 's_11_act_16_subact_02_ca_04' # outdoors_fencing_01
file_dir_bl = os.path.join('/home/chenkanghao/mywork/pose_estimate/I2L-MeshNet_onlymeshnetv3_2/output/vis', video_name)
...
video=cv2.VideoWriter('./'+video_name+'.avi',cv2.VideoWriter_fourcc(*'MJPG'),30,(2000,1002))  #定义保存视频目录名称及压缩格式，fps=10,像素为1280*720
...
# import os
# import cv2
# video_name = 'outdoors_fencing_01' # outdoors_fencing_01
# file_dir = os.path.join('../output/vis', video_name)
...
# video=cv2.VideoWriter('./'+video_name+'.avi',cv2.VideoWriter_fourcc(*'MJPG'),30,(1000,1002))  #定义保存视频目录名称及压缩格式，fps=10,像素为1280*720
# for i in range(1,len(img_list)):
#     img=cv2.imread(os.path.join(file_dir, img_list[i-1]))  #读取图片
#     img=cv2.resize(img,size) #将图片转换为1280*720
#     video.write(img)   #写入视频
