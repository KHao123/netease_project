...
class MeshNet(nn.Module):
    def __init__(self,
        ...
        self.backbone = timm.create_model('mobilenetv3_small_075', pretrained=True, num_classes=0, global_pool='')
        self.inplanes = 1024
        ...
        #     ConvBNReLU(2560, 2560, stride=1, groups=2560, norm_layer=nn.BatchNorm2d, activation_layer=nn.PReLU),
        #     nn.Conv2d(2560, 1280, 1, 1, 0, bias=False),
        #     nn.BatchNorm2d(1280)
        # )
        ...
        self.conv_z_1 = make_conv1d_layers([1024,256*64], kernel=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        # img_feat = torch.cat((img_feat, pose_feat),dim=1)
        # img_feat = self.conv_fuse(img_feat)
        # x =  self.backbone.conv_stem(x)
        # x = self.backbone.bn1(x)
        # x = self.backbone.act1(x)
        # x = self.backbone.blocks[1:](x)
        # x = self.backbone.conv_head(x)
        # x = self.backbone.bn2(x)
        # img_feat = self.backbone.act2(x)
        x = self.backbone.blocks[1:](x)
...
if __name__ == "__main__":
    ...
    model.eval()
    test_data = torch.rand(1, 3, 256, 256).cuda()
    pose_feat = torch.rand(1,1024,8,8).cuda()
    test_outputs = model(test_data, pose_feat)
