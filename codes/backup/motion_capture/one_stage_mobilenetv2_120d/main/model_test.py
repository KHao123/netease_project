...
class Model(nn.Module):
    def __init__(self, mesh_net, device='cuda'):
    ...
    def forward(self, inputs):
        ...
        #     if mode == 'train':
        #         # posenet forward
        #         # share_img_feat = self.share_backbone(inputs) #just for summary 
        #         mesh_coord_img = self.mesh_net(inputs['img'])
        #         # joint_coord_img = self.pose_net(share_img_feat)
        ...
        #         # mesh_coord_img = self.mesh_net(share_img_feat)
        ...
        #     else:
                # share_img_feat = self.share_backbone(inputs) #just for summary 
        mesh_coord_img = self.mesh_net(inputs)
                # mesh_coord_img = self.mesh_net(share_img_feat)
...
def get_model(vertex_num, joint_num, mode, device='cuda'):
    ...
    mesh_net = LpNetSkiConcat()
    # param_regressor = ParamRegressor(joint_num)
    # share_backbone = ResNetBackbone(cfg.resnet_type)
    # share_backbone = 
    ...
    # if mode == 'train':
        ...
        # share_backbone.init_weights()
   
    model = Model(mesh_net, device)
    return model
