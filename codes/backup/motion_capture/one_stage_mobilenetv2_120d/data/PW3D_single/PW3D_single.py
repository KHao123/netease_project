...
class PW3D_single(torch.utils.data.Dataset):
    def __init__(self, transform, data_split):
        self.transform = transform
        self.data_split = 'test'
        self.data_path = osp.join('/home/chenkanghao/mywork/pose_estimate/datasets', 'PW3D', 'data')
        self.human_bbox_root_dir = osp.join('/home/chenkanghao/mywork/pose_estimate/datasets', 'PW3D', 'rootnet_output', 'bbox_root_pw3d_output.json')
        ...
        self.h36m_joint_regressor = np.load(osp.join('/home/chenkanghao/mywork/pose_estimate/datasets', 'Human36M', 'J_regressor_h36m_correct.npy'))
        self.video_name = 'fencing' # fencing(test)  outdoors_slalom_00(train)
        '''
        {'downtown_walkBridge_01', 'downtown_arguing_00', 'downtown_cafe_00', 'downtown_warmWelcome_00', 'downto
        wn_sitOnStairs_00', 'downtown_crossStreets_00', 'office_phoneCall_00', 'flat_guitar_01', 'downtown_enter
        Shop_00', 'downtown_rampAndStairs_00', 'downtown_car_00', 'downtown_walking_00', 'flat_packBags_00', 'ou
        tdoors_fencing_01', 'downtown_bar_00', 'downtown_runForBus_01', 'downtown_stairs_00', 'downtown_runForBu
        s_00', 'downtown_bus_00', 'downtown_downstairs_00', 'downtown_walkUphill_00', 'downtown_weeklyMarket_00'
        , 'downtown_windowShopping_00', 'downtown_upstairs_00'}                                                 
        '''
        
        # self.testvideo = set()
        self.datalist = self.load_data()
        
        print('===>length of video:', len(self.datalist))
        # print(self.testvideo)
    ...
    def load_data(self):
        ...
        for aid in db.anns.keys():
            ...
            #选一个视频 200 帧
            # self.testvideo.add(img_path.split('/')[-2])
            if not (self.video_name in img_path.split('/')[-2] and len(datalist) < 1000):
                continue
    ...
    def evaluate(self, outs, cur_sample_idx):
        ...
        for n in range(sample_num):
            ...
            # print(annot['img_path'].split('/')[-2])
            # if 'fencing' in annot['img_path'].split('/')[-2]:
            #     vis = True
            # else:
            #     vis = False
            #     continue
            ...
            if vis:
                ...
                filename =  img_name + '_' + str(n)

                img = load_img(annot['img_path'])[:,:,::-1]
                # origin_img = img.copy()
                # img = vis_mesh(img, mesh_out_img, 0.5)
                # cv2.imwrite(os.path.join(cfg.vis_dir, filename + '.jpg'), img)

                # save_obj(mesh_out_cam, self.smpl.face, os.path.join(cfg.vis_dir, filename + '.obj'))
                
                rendered_img = render_mesh(img, mesh_out_cam, self.smpl.face, {'focal': focal, 'princpt': princpt})

                if not os.path.exists(os.path.join(cfg.vis_dir, seq_name)):
                    os.mkdir(os.path.join(cfg.vis_dir, seq_name))

                cv2.imwrite(os.path.join(cfg.vis_dir, seq_name, filename + '.jpg'), rendered_img)
