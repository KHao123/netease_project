...
class LpNetSkiConcat(nn.Module):
    ...
    def forward(self, x, skip_early=False):
        ...
        x = self.inv_residual[0:3](F1) # [1, 48, 16, 16]
        x2 = x
        x = self.inv_residual[3:10](x) # [1, 64, 8, 8]
        x1 = x
        x = self.inv_residual[10:](x) #[1, 96, 4, 4] [1, 96, 8, 8]
        # x = self.inv_residual[13:16](x) # [1, 160, 4, 4]
        x0 = x
        # x = self.inv_residual[16:](x) # [1, 320, 4, 4]
        img_feat = self.last_conv(x) # [1, 2048, 4, 4] [1, 2048, 8, 8]
