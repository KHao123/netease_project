...
class Pose2Feat_small(nn.Module):
    def __init__(self, joint_num):
        ...
        self.joint_num = joint_num
        # self.conv = make_conv_layers([64+joint_num*cfg.output_hm_shape[0],64])
        # self.conv = make_deconv_layers([64+joint_num*cfg.output_hm_shape[0], 48, 48])

        norm_layer = nn.BatchNorm2d
        activation_layer = nn.PReLU
        self.deconv1 = DeConv_v2(64+joint_num*cfg.output_hm_shape[0], 32, 48, norm_layer=norm_layer, activation_layer=activation_layer)
        # self.deconv2 = DeConv_v2(48, 32, 48, norm_layer=norm_layer, activation_layer=activation_layer)


        self.conv1 = make_conv_layers([48,64], kernel=3, stride=2)

    def forward(self, img_feat, joint_heatmap_3d): 
        #[1, 48, 128, 128]+[16, 29, 64, 64, 64] ->   [1, 48, 128, 128]
        joint_heatmap_3d = joint_heatmap_3d.view(-1,self.joint_num*cfg.output_hm_shape[0],cfg.output_hm_shape[1],cfg.output_hm_shape[2])
