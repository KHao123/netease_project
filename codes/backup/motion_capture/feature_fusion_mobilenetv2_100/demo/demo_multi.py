...
for n in range(person_num):
    ...
    img = transform(img.astype(np.float32))/255 # (256, 256, 3)
    img = img.cuda()[None,:,:,:]
    ...
    mesh_lixel_img = out['mesh_coord_img'][0].cpu().numpy() #(6890, 3) 每个坐标的都是相对于64的：output_hm_shape
    mesh_param_cam = out['mesh_coord_cam'][0].cpu().numpy() # (6890, 3)

    # restore mesh_lixel_img to original image space and continuous depth space
    mesh_lixel_img[:,0] = mesh_lixel_img[:,0] / cfg.output_hm_shape[2] * cfg.input_img_shape[1] # x-aixs
    mesh_lixel_img[:,1] = mesh_lixel_img[:,1] / cfg.output_hm_shape[1] * cfg.input_img_shape[0] # y-aixs
    mesh_lixel_img[:,:2] = np.dot(bb2img_trans, np.concatenate((mesh_lixel_img[:,:2], np.ones_like(mesh_lixel_img[:,:1])),1).transpose(1,0)).transpose(1,0) #将crop的结果转回原图？
    mesh_lixel_img[:,2] = (mesh_lixel_img[:,2] / cfg.output_hm_shape[0] * 2. - 1) * (cfg.bbox_3d_size / 2) # z-aixs

    # root-relative 3D coordinates -> absolute 3D coordinates 这部分操作看不懂
    
    root_xy = np.dot(joint_regressor, mesh_lixel_img)[root_joint_idx,:2]
    root_depth = root_depth_list[n] # obtain this from RootNet (https://github.com/mks0601/3DMPPE_ROOTNET_RELEASE/tree/master/demo)
    ...
    mesh_lixel_img_list.append(mesh_lixel_img)
    mesh_lixel_cam_list.append(mesh_lixel_cam)
    mesh_param_cam_list.append(mesh_param_cam)

vis_img = original_img.copy()
for n in range(person_num):
    mesh_lixel_img = mesh_lixel_img_list[n]
    vis_img = vis_mesh(vis_img, mesh_lixel_img)
cv2.imwrite('output_mesh_lixel_multi.jpg', vis_img)

vis_img = original_img.copy()
for n in range(person_num):
    mesh_lixel_cam = mesh_lixel_cam_list[n]
    vis_img = render_mesh(vis_img, mesh_lixel_cam, face, {'focal': focal, 'princpt': princpt})
cv2.imwrite('rendered_mesh_lixel_multi.jpg', vis_img)

# render mesh from param
vis_img = original_img.copy()
for n in range(person_num):
    mesh_param_cam = mesh_param_cam_list[n]
    vis_img = render_mesh(vis_img, mesh_param_cam, face, {'focal': focal, 'princpt': princpt})
cv2.imwrite('rendered_mesh_param_multi.jpg', vis_img)
