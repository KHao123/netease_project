...
from nets.pose_net import PoseNet
from nets.mesh_net import MeshNet

class Model(nn.Module):
    def __init__(self, pose_net, mesh_net, param_regressor):
    ...
    def forward(self, inputs, targets=None, meta_info=None, mode='test'):
        ...
        with cm:
            ...
            post_feat, joint_coord_img = self.pose_net(inputs['img']) #[32, 48, 256, 256]  [32, 2048, 8, 8]
            # joint_coord_img = self.pose_net(pose_img_feat)
            
            # make 3D heatmap from posenet output and convert to image feature
            # with torch.no_grad():
            #     joint_heatmap = self.make_gaussian_heatmap(joint_coord_img.detach()) # [16, 29, 64, 64, 64]
            # print(shared_img_feat.shape, joint_heatmap.shape)
            # shared_img_feat = self.pose2feat(inputs['img'], post_feat) # [1, 48, 128, 128]+[16, 29, 64, 64, 64] ->   [1, 48, 128, 128]

            # meshnet forward
            mesh_coord_img = self.mesh_net(inputs['img'], post_feat) 
...
def get_model_small(vertex_num, joint_num, mode):
    ...
    model = Model(pose_net, mesh_net, param_regressor)
    return model
