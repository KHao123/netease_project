...
from nets.module import PoseNet, Pose2Feat, MeshNet, ParamRegressor, PoseNet_small, Pose2Feat_small, MeshNet_small
...
import math
# from nets.lpnet_ski_concat import LpNetSkiConcat
from nets.lpnet_wo_concat import LpNetWoConcat

class Model(nn.Module):
    def __init__(self, pose_net, pose2feat, mesh_net, device='cuda'):
        ...
        if 'FreiHAND' in cfg.trainset_3d + cfg.trainset_2d + [cfg.testset]:
            ...
            self.human_model_layer = self.human_model.layer.to(self.device)
        else:
            self.human_model = SMPL()
            self.human_model_layer = self.human_model.layer['neutral'].to(self.device)
        ...
        self.joint_heatmap = torch.rand([1, 29, 64, 64, 64]).to(self.device).float()
        x = torch.arange(cfg.output_hm_shape[2])
        y = torch.arange(cfg.output_hm_shape[1])
        z = torch.arange(cfg.output_hm_shape[0])
        self.zz,self.yy,self.xx = torch.meshgrid(z,y,x)
        self.xx = self.xx[None,None,:,:,:].to(self.device).float(); self.yy = self.yy[None,None,:,:,:].to(self.device).float(); self.zz = self.zz[None,None,:,:,:].to(self.device).float()
    ...
    def make_gaussian_heatmap(self, joint_coord_img):
        ...
        heatmap = torch.exp(-(((self.xx-x)/cfg.sigma)**2)/2 -(((self.yy-y)/cfg.sigma)**2)/2 - (((self.zz-z)/cfg.sigma)**2)/2)
        return heatmap
    ...
    def forward(self, inputs):
        ...
        # with cm:
        # posenet forward
        shared_img_feat, joint_coord_img = self.pose_net(inputs) #just for summary 
        # shared_img_feat, joint_coord_img = self.pose_net(inputs['img']) #[32, 48, 256, 256]  [32, 2048, 8, 8]
        # joint_coord_img = self.pose_net(pose_img_feat)
        
        # make 3D heatmap from posenet output and convert to image feature
        # with torch.no_grad():
        # joint_heatmap = self.make_gaussian_heatmap(joint_coord_img) # [16, 29, 64, 64, 64]
        joint_heatmap = self.joint_heatmap
        # print(shared_img_feat.shape, joint_heatmap.shape)
        shared_img_feat = self.pose2feat(shared_img_feat, joint_heatmap) # [1, 48, 128, 128]+[16, 29, 64, 64, 64] ->   [1, 48, 128, 128]

        # meshnet forward
        _, mesh_coord_img = self.mesh_net(shared_img_feat, skip_early=True) 
...
def get_model_small(vertex_num, joint_num, mode, device='cuda'):
    # pose_backbone = LpNetSkiConcat()
    pose_net = LpNetWoConcat(vertex_num=joint_num)
    pose2feat = Pose2Feat_small(joint_num)
    # mesh_backbone = LpNetSkiConcat()
    mesh_net = LpNetWoConcat(vertex_num=vertex_num)
    # param_regressor = ParamRegressor(joint_num)
    ...
    model = Model(pose_net, pose2feat, mesh_net, device)
    return model
