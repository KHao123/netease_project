...
class Trainer(Base):
    ...
    def get_optimizer(self, model):
        if cfg.stage == 'lixel':
            optimizer = torch.optim.Adam(
                                        # list(model.module.pose_backbone.parameters()) + \
    ...
    def _make_model(self):
        ...
        model = get_model_small(self.vertex_num, self.joint_num, 'train')
...
class Tester(Base):
    ...
    def _make_model(self):
        ...
        model = get_model_small(self.vertex_num, self.joint_num, 'test')
        model = DataParallel(model).cuda()
