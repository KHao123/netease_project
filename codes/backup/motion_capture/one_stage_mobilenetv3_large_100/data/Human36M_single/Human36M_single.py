...
class Human36M_single(torch.utils.data.Dataset):
    def __init__(self, transform, data_split):
        ...
        ###
        self.img_dir = osp.join('/home/chenkanghao/mywork/pose_estimate/datasets', 'Human36M', 'images')
        self.annot_path = osp.join('/home/chenkanghao/mywork/pose_estimate/datasets', 'Human36M', 'annotations')
        self.human_bbox_root_dir = osp.join('/home/chenkanghao/mywork/pose_estimate/datasets', 'Human36M', 'rootnet_output', 'bbox_root_human36m_output.json')
        ...
        self.video_name = 's_09_act_04_subact_02_ca_04' #s_11_act_16_subact_02_ca_04
    ...
    def load_data(self):
        ...
        for aid in db.anns.keys():
            ...
            #选一个视频 200 帧
            # print(img_path.split('/')[-2], self.video_name)
            if self.video_name != img_path.split('/')[-2] or len(datalist) >= 4000:
                continue
    ...
    def evaluate(self, outs, cur_sample_idx):
        ...
        for n in range(sample_num):
            ...
            vis = True
            if vis:
                videoname = annot['img_path'].split('/')[-2]
                filename = annot['img_path'].split('/')[-1][:-4]

                img = load_img(annot['img_path'])[:,:,::-1]

                if not os.path.exists(os.path.join(cfg.vis_dir, videoname)):
                    os.mkdir(os.path.join(cfg.vis_dir, videoname))

                # mesh_img = vis_mesh(img, mesh_out_img, 0.5)
                # cv2.imwrite(os.path.join(cfg.vis_dir, videoname, filename + '.jpg'), mesh_img)

                # save_obj(mesh_out_cam, self.smpl.face, os.path.join(cfg.vis_dir, videoname, filename + '.obj'))
                mesh_out_cam /= 1000
                rendered_img = render_mesh(img, mesh_out_cam, self.smpl.face, {'focal': focal, 'princpt': princpt})
                cv2.imwrite(os.path.join(cfg.vis_dir, videoname, filename + '.jpg'), rendered_img)
